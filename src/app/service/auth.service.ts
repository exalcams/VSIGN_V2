import { TourStatusModel } from "src/app/models/TourStatusModel";
...
//import { ActionLog } from "src/app/models/OrderFulFilment";
...
export class AuthService {
  ...
  isLoggedin(change: boolean) {
    ...
    constructor(private _httpClient: HttpClient) {
        this.baseAddress ="http://192.168.0.25:9003/";
        this.clientId = environment.clientId;
        this.currentUserSubject = new BehaviorSubject<AuthenticationDetails>(JSON.parse(localStorage.getItem("authorizationData")));
    ...
    CreateActionLog(log: ActionLog): Observable<any> {
        return this._httpClient.post<ActionLog>(`http://192.168.0.25:9003/api/Master/CreateActionLog`,
            log,
                {
                    headers: new HttpHeaders({
                        "Content-Type": "application/json",
    ...
    GetAllActionLogs(): Observable<any> {
        return this._httpClient
            .get<ActionLogView[]>(
                `http://192.168.0.25:9003/api/Master/GetAllActionLogs`)
            .pipe(catchError(this.errorHandler1));
    }
    FilterASNList(UserName: string, AppName: string, UsedDate: string): Observable<any> {
        return this._httpClient
            .get<ActionLogView[]>(
                `http://192.168.0.25:9003/api/Master/FilterASNList?UserName=${UserName}&AppName=${AppName}&UsedDate=${UsedDate}`
            ).pipe(catchError(this.errorHandler));
    }
}
